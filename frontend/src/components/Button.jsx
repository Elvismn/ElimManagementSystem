...
const Button = ({ 
  ...
  const variants = {
    ...
    warning: 'bg-yellow-500 text-white hover:bg-yellow-600 focus:ring-yellow-500 disabled:hover:bg-yellow-500',
  ...
  const renderIcon = () => {
    if (!Icon) return null
    return <Icon className={`w-4 h-4 ${iconPosition === 'left' ? 'mr-2' : 'ml-2'}`} />
  }
